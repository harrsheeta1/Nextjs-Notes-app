...
app.use(express.static(path.join(__dirname, '../client'))); //making the client folder static so that it can be served
...
app.use(cors()); // Enable the frontend and backend to communicate without CORS issues even if they are on different ports or domains
...
const PORT = process.env.PORT || 5500; // Use PORT from environment or default to 5000
app.listen(PORT, () => {    
